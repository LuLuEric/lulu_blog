...
export function chooseAction(view) {
  ...
  const p = view.players[view.actor ?? view.active];
  ...
  const influenceChange = n => {
    const after = Math.max(0, Math.min(30, p.influence + n));
  ...
  const incomeDamage = (target, amount) => {
    const before = Math.floor(Math.max(0, target.loyalty - 5) / 20);
    const after = Math.floor(Math.max(0, target.loyalty - amount - 5) / 20);
    return (before - after) * Math.min(remaining, 6) * 0.45 + Math.min(target.loyalty, amount) * Math.min(remaining, 6) * 0.025;
  };
  if (view.phase === 'reaction') {
    const { card, actor } = view.pending;
    const damage = card === 'denounce' ? 10 : card === 'frame' ? 7 : card === 'unanimous' ? (p.loyalty < view.players[actor].loyalty ? 12 : 5) : 0;
    const protection = card === 'transfer' ? Math.min(remaining, 8) * 1.4 + 6 : Math.min(p.favor, damage) * favorValue + (card === 'frame' ? CARDS.frame.loyaltyDamage * loyaltyValue : 0);
    const block = view.legal.find(a => a.block);
    return block && protection > CARDS.shield.cost * influenceValue + 2 ? block : view.legal.find(a => !a.block);
  }
  ...
  const cardScore = (id, a = {}) => {
    ...
    switch (id) {
      ...
      case 'levy': {
        const amount = t ? Math.min(t.influence, sets(view, p.id) ? 8 : 5, 30 - p.influence + card.cost) : 5;
        value = influenceChange(amount - card.cost) + amount * 0.45; break;
      }
      case 'trade': value += favorGain(5) - card.gift * 0.2 + (a.giveIndex === undefined ? 1 : 4 - Math.max(0, cardScore(p.hand[a.giveIndex])) * 0.55); break;
      case 'transfer': {
      ...
      case 'frame': value += (t ? attack(t, 7) : 4) + (t?.shield ? 0 : t ? incomeDamage(t, card.loyaltyDamage) : Math.min(remaining, 6) * 0.35); break;
      case 'alibi': value += favorGain(7) + loyaltyGain(5); break;
      case 'unanimous': value += t ? attack(t, t.loyalty < p.loyalty ? 12 : 5) : 4; break;
      case 'retreat': value += favorGain(4); break;
    }
    return value;
  };
  function score(a) {
