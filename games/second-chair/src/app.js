...
function coachPanel() {
  ...
  if (guide.coach === 'stats') {
    ...
    text = '点击金色边框的手牌，先读效果，再看“执行后预览”。确认才会出牌；打牌需要卡面费用，还占用 1 次行动。你也可以打其他牌。';
...
function cardView(id, index, large = false, readOnly = false) {
  ...
  return `<${tag} class="hand-card ${card.color} ${large ? 'large' : ''} ${!large && !readOnly && guidedCard(index) ? 'guide-highlight' : ''} ${!playable && !large && !readOnly ? 'unavailable' : ''}" ${readOnly ? '' : `data-card="${index}" aria-label="查看手牌 ${card.name}，费用 ${card.cost} 影响力"`}>
...
function actionPreview(action) {
  ...
  return `<div class="action-preview"><strong>执行后预览</strong><div>${[['loyalty', '忠诚'], ['influence', '影响力'], ['favor', '宠幸']].map(([key, label]) => `<span>${label}<b>${before[key]} → ${after[key]}</b></span>`).join('')}</div><p class="action-budget">剩余行动：${game.actions} → ${game.actions - 1} 次 · 本操作占用 1 次行动</p><p>本轮峰值 ${after.peak} · 轮末预计损失 ${forecast(after)} 宠幸</p></div>`;
}
...
function drawDialog() {
  ...
  if (d.kind === 'guide') {
    const pages = [
      ...
      `${dialogHeader('一次行动，先看两笔账', '专员入职简报 · 3 / 3 · 准备动手')}<p class="guide-lead">轮到你时，摸 2 张牌，最多行动 2 次。<strong>影响力是预算，行动次数是本次能办几件事。</strong></p><div class="guide-example"><span class="eyebrow">示例 · 忠诚表态</span><h3>花 2 影响力，占 1 次行动</h3><div><span>影响力 <b>10 → 8</b></span><span>剩余行动 <b>2 → 1</b></span></div><p>获得 15 忠诚；结算后忠诚最高（含并列），再得 3 宠幸。这只是示例，实际结果会按你的对局预览。</p></div><ol class="guide-steps"><li>点击一张手牌，读效果和费用。</li><li>需要目标时先选同僚或机构，检查数值预览。</li><li>确认出牌；准备好后点击“结束行动”。</li></ol><p class="muted">机构经营、轮末结算、弃牌和调查，会在遇到时解释。右上角随时可重看引导。</p>`,
    ];
    ...
    const detail = d.key === 'loyalty' ? `每轮收入 = 2 + ⌊忠诚 ÷ 20⌋ + 机构数 + 完整机构组数。你当前每轮收入 ${income(game, 0)} 影响力；忠诚每轮结束下降 5。` : d.key === 'influence' ? `影响力上限 30。本轮峰值 ${p.peak}，轮末目前预计损失 ${forecast(p)} 宠幸；花掉资源不会降低已记录的峰值。猜忌 = ⌈max(0, 峰值 − 12) ÷ 3⌉，再加固定损失 3。` : `宠幸上限 100；普通轮末至少下降 3。${p.investigation ? investigationDeadline(game) : '低于 20 进入调查，得到完整行动机会后仍未达标会被清洗。'}最终先结清表忠债务和审查，再按宠幸排名；同分比较忠诚。`;
    ...
    modal.innerHTML = `${dialogHeader(c.name, '你的手牌 · 消耗 1 次行动')}<div class="card-dialog-layout">${cardView(id, d.index, true, true)}<div class="card-dialog-controls">${guide.enabled && guide.coach === 'card' ? '<div class="learn-note">先看费用：<b>' + c.cost + ' 影响力 + 1 次行动</b>。下方预览包含卡牌效果；确认后才真正执行。' + (c.target ? '这张牌还需要先选择目标。' : '') + '</div>' : ''}<p class="dialog-intro">${c.text}</p>${options}${confirmation(action, `打出卡牌 · ${c.cost} 影响力`)}</div></div>`;
    ...
    modal.innerHTML = `${dialogHeader('如何坐上第二把椅子', '玩法说明 · 先理解目标，再查细节')}<p class="guide-lead">与 3 名电脑竞争 12 轮。<strong>通过最终审查后，宠幸最高的存活者获胜。</strong></p>${concepts()}${conceptFlow()}<section class="rules-basics"><h3>轮到你，现在做什么</h3><ol class="guide-steps"><li>获得 2 张手牌和 2 次行动。点击手牌看效果、费用，必要时选择目标，确认后执行。</li><li>也可以任命机构积累下轮收入，或公开检讨争取宠幸。这些各占 1 次行动。</li><li>不必用完行动，点击“结束行动”交给下一位。手牌超过 7 张时先选择弃牌。</li></ol></section><details class="rule-detail"><summary>一轮如何结算 · 收入、行动与衰减</summary><p>公开事件 → 全员收入 → 按轮换顺序行动 → 轮末结算。每人行动结束不等于一轮结束；全部存活官员完成后才统一扣除忠诚和宠幸。</p><p>每轮收入 = 2 + ⌊忠诚 ÷ 20⌋ + 机构数 + 完整机构组数。忠诚每轮结束 −5；影响力上限 30，忠诚与宠幸上限 100。</p></details><details class="rule-detail"><summary>为什么权力大了反而失宠 · 峰值与猜忌</summary><p>本轮影响力最高值超过 12 会招来猜忌，花掉影响力也不会抹去峰值。轮末宠幸固定 −3，再扣猜忌。</p><div class="rule-note">猜忌 = ⌈max(0, 峰值 − 12) ÷ 3⌉。例如峰值 19，轮末共损失 6 宠幸。</div></details><details class="rule-detail"><summary>机构如何经营 · 任命、成套、征收与交权</summary><p>无主机构任命花 5 影响力与 1 次行动，从下一轮开始每轮收入 +1。同系统两处成套再 +1，每人最多 3 处。使用「专项摊派」向同僚征收资源，「人事调动」接管对手机构。</p><p>主动交出一处机构获得 9 宠幸，占 1 次行动。「挡箭牌」防 1 次定向政治攻击，下一轮开始过期。</p></details><details class="rule-detail"><summary>宠幸过低怎么自救 · 调查与终局</summary><p>宠幸低于 20 进入调查。获得完整行动机会后，期限结束仍未恢复到 20 就被清洗。调查提示会标明你的实际期限。</p><p>公开检讨消耗 10 忠诚，获得 5 宠幸；交权获得 9 宠幸；也可使用自保或宣传牌。第 12 轮先清偿全部延迟债务，再审查，宠幸低于 20 无法继承。</p><p>最终宠幸相同看忠诚，再同分则并列。所有人被清洗，元首独自获胜。</p></details><details class="card-catalog"><summary>查看全部 16 种行动牌</summary><div class="catalog-grid">${Object.values(CARDS).map(c => `<div><strong>${icon(c.icon)}${c.name}<b>${c.cost}</b></strong><p>${c.text}</p></div>`).join('')}</div></details><p class="muted">电脑与玩家使用相同规则，不读取对手手牌。进度保存在当前浏览器；刷新可以继续。本作人物、国家和机构均属虚构。</p>`;
  }
}
...
function takeAction(action, showToast = true) {
  ...
  if (previous.active === 0 && guide.enabled && guide.coach !== 'done') {
...
function aiPaused() { return game.finished || game.active === 0 || modal.open || (guide.enabled && guide.coach === 'stats'); }
function scheduleAI() {
