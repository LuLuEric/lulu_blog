...
function render() {
  ...
  app.innerHTML = `<div class="shell">
    ...
    <div class="layout"><main class="table-area">
      ...
      <section class="risk-panel panel ${projected < 25 || p.investigation ? 'at-risk' : ''}"><div class="section-title"><h2>${icon('eye')}元首的目光</h2><span>${p.investigation ? '正在调查' : projected < 25 ? '保持警惕' : '暂时安全'}</span></div><div class="risk-numbers"><span>本轮权势峰值<strong>${p.peak}<small> / 30</small></strong></span><span>轮末宠幸预计<strong class="${loss > RULES.favorDecay ? 'danger-text' : ''}">−${loss}</strong></span></div><div class="risk-meter"><i style="width:${p.peak / 30 * 100}%"></i><b style="left:40%"></b></div><p>折旧 ${RULES.favorDecay} + 猜忌 ${loss - RULES.favorDecay} <span>·</span> 当前预计剩余 <b>${projected}</b> 宠幸</p>${p.investigation ? '<div class="investigation-alert">' + icon('eye') + investigationDeadline(game) + '</div>' : '<small class="muted">影响力超过 12 开始引起猜忌。花掉资源不会降低本轮峰值。</small>'}${p.debts.length ? '<div class="investigation-alert">' + icon('clock') + '待偿还表忠代价：' + p.debts.reduce((n, d) => n + d.amount, 0) + ' 宠幸</div>' : ''}</section>
    ...
    </aside></div><footer class="footer"><span>所有人都不可替代，直到下一次人事调整。</span><span>会议编号 ${escape(game.seed)} · 牌组 v0.2.1</span></footer>
  </div>`;
...
function drawDialog() {
  ...
  if (d.kind === 'reaction') {
    ...
    const detail = d.key === 'loyalty' ? `每轮收入 = 2 + ⌊忠诚 ÷ 20⌋ + 机构数 + 完整机构组数。忠诚每轮结束下降 5。已计这次下降、未计未来事件时，你下轮预计收入 ${nextIncome(game, 0)} 影响力；第 12 轮之后不再发收入。` : d.key === 'influence' ? `影响力上限 30。本轮峰值 ${p.peak}，轮末目前预计损失 ${forecast(p)} 宠幸；花掉资源不会降低已记录的峰值。猜忌 = ⌈max(0, 峰值 − 12) ÷ 3⌉，再加固定损失 ${RULES.favorDecay}。` : `宠幸上限 100；普通轮末至少下降 ${RULES.favorDecay}。${p.investigation ? investigationDeadline(game) : '低于 20 进入调查，得到完整行动机会后仍未达标会被清洗。'}最终先完成轮末结算和审查，再按宠幸排名；同分比较忠诚。旧会议已有的表忠欠款仍须清偿。`;
    ...
    const copy = d.lesson === 'end' ? {
      ...
      note: `按当前峰值 ${p.peak}，你本轮预计损失 ${loss} 宠幸（固定 ${RULES.favorDecay} + 猜忌 ${loss - RULES.favorDecay}）。同僚接下来的行动仍可能改变结果。下一轮先发生公共事件，再发收入。`,
      ...
      text: `你的本轮影响力峰值已达到 ${p.peak}，超过 12 开始招来猜忌。轮末目前预计扣 ${loss} 宠幸（固定 ${RULES.favorDecay} + 猜忌 ${loss - RULES.favorDecay}）。`,
